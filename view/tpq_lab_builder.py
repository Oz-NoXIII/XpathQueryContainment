  ...
  <header>
     <div class="header-top">
       <div class="header-copy">
         <h1>$title</h1>
         <p class="hint">Construisez deux <strong>BoolTPQ_Lab</strong> côte à côte, puis cliquez sur <strong>Vérifier le containment</strong> pour tester si <code>q1</code> est contenu dans <code>q2</code>. Les labels sont obligatoires et les relations utilisent uniquement <code>child</code> et <code>descendant</code>.</p>
       </div>
       $theme_switch
     </div>
    <div class="toolbar">
      <button type="button" id="find-homomorphism">Vérifier le containment</button>
...
$theme_script
    ...
    function candidateTargets(targetState, targetParentNode, edgeType) { if (edgeType === 'child') { return getChildren(targetState, targetParentNode.id).filter(function(child) { return child.edgeType === 'child'; }); } var result = []; function collect(node) { getChildren(targetState, node.id).forEach(function(child) { result.push(child); collect(child); }); } collect(targetParentNode); return result; }
    ...
    function setSelectedNodeLabel(state, value) { var selected = getNode(state, state.selectedId); if (!selected) { return; } var label = value.trim() || 'a'; selected.label = label; renderQuery(state); }
    function addNode(state, edgeType) { var parent = getNode(state, state.selectedId); if (!parent) { setStatus(state, "Sélectionnez d'abord un nœud.", 'is-error'); return; } var label = window.prompt('Label du nouveau nœud', 'a'); if (label === null) { return; } label = label.trim() || 'a'; var newNode = { id: 'node_' + state.nextId, label: label, parentId: parent.id, edgeType: edgeType, x: parent.x + (edgeType === 'child' ? 140 : 110), y: parent.y + 120 }; state.nextId += 1; state.nodes.push(newNode); state.selectedId = newNode.id; layoutQuery(state); setStatus(state, 'Nœud ' + newNode.id + ' ajouté.', 'is-success'); renderQuery(state); }
    function renameSelected(state) { var selected = getNode(state, state.selectedId); if (!selected) { return; } var label = window.prompt('Nouveau label', selected.label); if (label === null) { return; } label = label.trim() || 'a'; selected.label = label; setStatus(state, 'Nœud ' + selected.id + ' renommé.', 'is-success'); renderQuery(state); }
    ...
    boot();
      ...
      <div class=\"query-side\">
		...
		query_body_2 = query_body_1.replace('q1', 'q2').replace('Cible du containment (q1 ⊆ q2?)', 'Source du containment (q1 ⊆ q2?)')
